...
- **The New Ragnarok Reborn — GM Guides** (Journal pack) — five entries:
  ...
  *Vorath's Hellheim Throne Room — Scene Guide* (its infernal twin: braziers, chasm,
  ...
  `Hellheim Throne Room Scene Guide.md`). Plus the *10-Minute Playtest Checklist —
  Every Activity*: 46 checkbox rows covering every activity on all five actors and both
  magic items, each stating the exact bonus, dice, DC and effect the chat card must
  show — so data bugs are caught by rolling, not by reading JSON.
...
- **One-import demo adventure**: a new *Demo Adventure* pack bundles the five
  ...
  the import can't half-bind (the generator refuses a bundle whose scene
  tokens reference an actor `_id` that isn't included). Generated by
  `tools/gen-adventure.py` from the same sources everything else uses.
- **Release gate pins the demo inventory**: the verifier asserts the compiled
  adventure embeds exactly 5 actors / 2 scenes / 2 items / 7 journals, each
  matching its source, alongside the existing checklist-shape pin.
...
- **The playtest checklist is now derived, not hand-listed**: `tools/gen-playtest.py`
  ...
  Add an activity to an actor, rerun the script, and the row appears automatically
...
- **New GM Guides entry: *10-Minute Playtest Checklist — Every Activity*** — a timed
  ...
  the exact expected chat-card output per row. Generated by `tools/gen-playtest.py`;
  the documented-data CI gate now asserts the checklist against the actors in both
  directions (42/42 claims covered), so the checklist can't silently drift from the
  shipped data.
